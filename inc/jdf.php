...
/*	F	*/
function s_jdate($format,$timestamp='',$none='',$time_zone='Asia/Tehran',$tr_num='en'){
 ...
 list($j_y,$j_m,$j_d)=s_gregorian_to_jalali($date[8],$date[3],$date[2]);
 ...
 for($i=0; $i<$sl; $i++){
  ...
  switch($sub){
	...
	case'D':
	$out.=s_jdate_words(array('kh'=>$date[7]),' ');
	break;

	case'f':
	$out.=s_jdate_words(array('ff'=>$j_m),' ');
	break;

	case'F':
	$out.=s_jdate_words(array('mm'=>$j_m),' ');
	break;
	...
	case'J':
	$out.=s_jdate_words(array('rr'=>$j_d),' ');
	break;
	...
	case'l':
	$out.=s_jdate_words(array('rh'=>$date[7]),' ');
	break;
	...
	case'M':
	$out.=s_jdate_words(array('km'=>$j_m),' ');
	break;
	...
	case'p':
	$out.=s_jdate_words(array('mb'=>$j_m),' ');
	break;
	...
	case'q':
	$out.=s_jdate_words(array('sh'=>$j_y),' ');
	break;
	...
	case'r':
	$key=s_jdate_words(array('rh'=>$date[7],'mm'=>$j_m));
	...
	case'v':
	 $out.=s_jdate_words(array('ss'=>substr($j_y,2,2)),' ');
	break;

	case'V':
	$out.=s_jdate_words(array('ss'=>$j_y),' ');
	break;
...
function jstrftime($format,$timestamp='',$none='',$time_zone='Asia/Tehran',$tr_num='fa'){
 ...
 list($j_y,$j_m,$j_d)=s_gregorian_to_jalali($date[7],$date[4],$date[3]);
 $doy=($j_m<7)?(($j_m-1)*31)+$j_d-1:(($j_m-7)*30)+$j_d+185;
 $kab=($j_y%33%4-1==(int)($j_y%33*.05))?1:0;
 $sl=strlen($format);
 $out='';
 for($i=0; $i<$sl; $i++){
  ...
  switch($sub){
	...
	/* Day */
	case'a':
	$out.=s_jdate_words(array('kh'=>$date[6]),' ');
	break;

	case'A':
	$out.=s_jdate_words(array('rh'=>$date[6]),' ');
	break;
	...
	case'c':
	$key=s_jdate_words(array('rh'=>$date[6],'mm'=>$j_m));
...
function jmktime($h='',$m='',$s='',$jm='',$jd='',$jy='',$is_dst=-1){
 ...
 if($h=='' and $m=='' and $s=='' and $jm=='' and $jd=='' and $jy==''){
	...
	list($year,$month,$day)=s_jalali_to_gregorian($jy,$jm,$jd);
...
function jgetdate($timestamp='',$none='',$tz='Asia/Tehran',$tn='en'){
 ...
 $s_jdate=explode('_',s_jdate('F_G_i_j_l_n_s_w_Y_z',$ts,'',$tz,$tn));
 return array(
	'seconds'=>tr_num((int)tr_num($s_jdate[6]),$tn),
	'minutes'=>tr_num((int)tr_num($s_jdate[2]),$tn),
	'hours'=>$s_jdate[1],
	'mday'=>$s_jdate[3],
	'wday'=>$s_jdate[7],
	'mon'=>$s_jdate[5],
...
/*	F	*/
function s_jcheckdate($jm,$jd,$jy){
...
/*	F	*/
function s_jdate_words($array,$mod=''){
...
/*	F	*/
function s_gregorian_to_jalali($g_y,$g_m,$g_d,$mod=''){
...
/*	F	*/
function s_jalali_to_gregorian($j_y,$j_m,$j_d,$mod=''){
